...
DO it = Ttransition,1,-1
	...
	ELSEIF(SolveFlexPriceTransition==1) THEN
		irfpointer%solnFLEX(it)%mpc = mpc
		irfpointer%solnFLEX(it)%subeff1ass  = subeff1ass
		irfpointer%solnFLEX(it)%subeff2ass  = subeff2ass
		irfpointer%solnFLEX(it)%wealtheff1ass  = wealtheff1ass
		irfpointer%solnFLEX(it)%wealtheff2ass  = wealtheff2ass
	ELSEIF(SolveZLBTransition==1) THEN
		irfpointer%solnZLB(it)%mpc = mpc
		irfpointer%solnZLB(it)%subeff1ass  = subeff1ass
		irfpointer%solnZLB(it)%subeff2ass  = subeff2ass
		irfpointer%solnZLB(it)%wealtheff1ass  = wealtheff1ass
		irfpointer%solnZLB(it)%wealtheff2ass  = wealtheff2ass
	END IF	
END DO
