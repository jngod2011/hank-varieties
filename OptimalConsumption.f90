...
!add this option: IF((testingGHH == .true.) .or. (NoChangeLabDisutility==1))
...
IF (lVb > -999.0) THEN !not at stationary point or limit
	...
	ELSE IF	(LaborSupplySep==1) THEN
		IF(ScaleDisutilityIdio==0) THEN
			IF(prodispshock==.false. .or. ProdDispScaleDisutilty==0) lh = (labwedge*gnetwage*lVb/chi)**(frisch)
! 			IF(prodispshock==.true. .and. ProdDispScaleDisutilty==1) lh = ((labwedge*gnetwage/gidioscale)*lVb/chi)**(frisch)
			IF(prodispshock==.true. .and. ProdDispScaleDisutilty==1) lh = ((labwedge*gnetwage*gidioprodSS/gidioprod)*lVb/chi)**(frisch)
		END IF
		IF(ScaleDisutilityIdio==1) lh = ((labwedge*gnetwage/gidioprod)*lVb/chi)**(frisch)
		IF(ImposeMaxHours==1)lh = min(lh, 1.0_8)
		
		
		IF(ScaleDisutilityIdio==0) THEN
			IF(prodispshock==.false. .or. ProdDispScaleDisutilty==0) llabdisutil = lh**(1.0+1.0/frisch)/(1.0+1.0/frisch)
! 			IF(prodispshock==.true. .and. ProdDispScaleDisutilty==1) llabdisutil = gidioscale * (lh**(1.0+1.0/frisch)/(1.0+1.0/frisch))
			IF(prodispshock==.true. .and. ProdDispScaleDisutilty==1) llabdisutil = (gidioprod/gidioprodSS) * (lh**(1.0+1.0/frisch)/(1.0+1.0/frisch))
		END IF	
		IF(ScaleDisutilityIdio==1) llabdisutil = gidioprod * lh**(1.0+1.0/frisch)/(1.0+1.0/frisch)
		lc = utilfn1inv(lVb)
		ls = gbdrift + lh*gnetwage -lc
		IF(lc>0.0) THEN
			lHc = utilfn(lc) - (chi/labwedge)*llabdisutil + lVb*ls
		ELSE
			lHc = -1.0e12
		END IF
		
	ELSE IF	(LaborSupplyGHH==1) THEN
		IF(ScaleDisutilityIdio==0) lh = (labwedge*gnetwage/chi)**(frisch)
		IF(ScaleDisutilityIdio==1) lh = ((labwedge*gnetwage/gidioprod)/chi)**(frisch)
		IF(ImposeMaxHours==1) lh = min(lh, 1.0_8)
		IF(ScaleDisutilityIdio==0) llabdisutil = lh**(1.0+1.0/frisch)/(1.0+1.0/frisch)
		IF(ScaleDisutilityIdio==1) llabdisutil = gidioprod * lh**(1.0+1.0/frisch)/(1.0+1.0/frisch)
		lc = utilfn1inv(lVb) + (chi/labwedge)*llabdisutil
		ls = gbdrift + lh*gnetwage -lc
		IF(lc-(chi/labwedge)*llabdisutil>0.0) THEN
			lHc = utilfn(lc-(chi/labwedge)*llabdisutil) + lVb*ls
		ELSE
			lHc = -1.0e12
		END IF
...
! 			IF(prodispshock==.true. .and. ProdDispScaleDisutilty==1) llabdisutil =  gidioscale * (lh**(1.0+1.0/frisch)/(1.0+1.0/frisch))
		...
		lc = gbdrift + lh*gnetwage
		IF(lc>0.0) THEN
			lHc = utilfn(lc) - (chi/labwedge)*llabdisutil
		ELSE
			lHc = -1.0e12
		END IF
