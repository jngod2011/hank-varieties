...
IF(FixedValueForD==1) THEN
	IF(ld>0.0) adjcostfn = kappafc_d
	IF(ld<0.0) adjcostfn = kappafc_w
	RETURN	
END IF
	
IF(AdjustCostFnRatio ==1 .and. Kappa3LinearOrMax==1) lx = ld/(kappa3+la)
IF(AdjustCostFnRatio ==1 .and. Kappa3LinearOrMax==2) lx = ld/max(kappa3,la)
IF(AdjustCostFnRatio ==0) lx = ld

IF (ExponAdjustConstFn==1 ) THEN
	IF(lx==0.0) adjcostfn = 0.0
	IF(lx>0.0) adjcostfn = kappafc_d + (1.0/kappa4_d)*(exp(kappa4_d*lx)-1.0) -lx
	IF(lx<0.0) adjcostfn = kappafc_w + (1.0/kappa4_w)*(exp(kappa4_w*lx)-1.0) -lx
	
ELSE
	IF(lx == 0.0_8) THEN
		adjcostfn = 0.0
	ELSE IF (lx>0.0_8) THEN
	! 	adjcostfn = kappa0_d*abs(lx) + (abs(lx/kappa1_d)**(1.0+kappa2_d)) *kappa1_d / (1.0+kappa2_d)
		adjcostfn = kappa0_d*abs(lx) + (abs(lx)**(1.0+kappa2_d)) *(kappa1_d**(-kappa2_d)) / (1.0+kappa2_d)
		IF(AdjustCostFnRatio ==1 .and. Kappa3LinearOrMax==1) adjcostfn = adjcostfn*(kappa3+la)
		IF(AdjustCostFnRatio ==1 .and. Kappa3LinearOrMax==2) adjcostfn = adjcostfn*max(kappa3,la)
		adjcostfn = kappafc_d + adjcostfn
	ELSE IF (lx<0.0_8) THEN
	! 	adjcostfn = kappa0_w*abs(lx) + (abs(lx/kappa1_w)**(1.0+kappa2_w)) *kappa1_w  / (1.0+kappa2_w)
		adjcostfn = kappa0_w*abs(lx) + (abs(lx)**(1.0+kappa2_w)) *(kappa1_w**(-kappa2_w))  / (1.0+kappa2_w)
		IF(AdjustCostFnRatio ==1 .and. Kappa3LinearOrMax==1) adjcostfn = adjcostfn*(kappa3+la)
		IF(AdjustCostFnRatio ==1 .and. Kappa3LinearOrMax==2) adjcostfn = adjcostfn*max(kappa3,la)
		adjcostfn = kappafc_w + adjcostfn
	END IF
END IF
...
IF (ExponAdjustConstFn==1 ) THEN
	IF(lx>0.0) adjcostfn1 = exp(kappa4_d*lx)-1.0
	IF(lx<0.0) adjcostfn1 = exp(kappa4_w*lx)-1.0
ELSE
...
IF(FixedValueForD==1) THEN
	IF(lchi==0.0) adjcostfn1inv = 0.0
	IF(lchi>0.0) adjcostfn1inv = dexog
	IF(lchi<0.0) adjcostfn1inv = -dexog
	RETURN	
END IF
...
IF (ExponAdjustConstFn==1 ) THEN
	IF(lchi==0.0) adjcostfn1inv = 0.0
	IF(lchi>0.0) adjcostfn1inv = (1.0/kappa4_d) * log(1.0+lchi)
	IF(lchi<0.0) adjcostfn1inv = (1.0/kappa4_w) * log(1.0+lchi)
