...
%%
% OutputDir = '/Volumes/FILES/Projects/MollContinuousTime/Results/Test7x7';
% OutputDir = '/Volumes/FILES/Projects/MollContinuousTime/Results/Test/';
OutputDir = '/Volumes/FILES/Large/ContinuousTimeAdjustment/Test';
SaveDir = '/Volumes/FILES/Dropbox/MiscellaneousShared/HANK/ModelOutput/Figures26Feb/SteadyState';
MakeAllFigs = 1;
Save = 0;
...
for iy = 1:ngpy
    ...
    gjoint(:,:,iy) = load([OutputDir '/INITSS/gjoint_INITSS_y' int2str(iy) '.txt']);    
end    
gamarg = load([OutputDir '/INITSS/gamarg_INITSS.txt']);    
gbmarg = load([OutputDir '/INITSS/gbmarg_INITSS.txt']);
...
%%
mpc = (con(:,2:ngpb,:) - con(:,1:ngpb-1,:))./ repmat(bdelta(1:ngpb-1)',ngpa,1,ngpy);
mpc = [mpc mpc(:,ngpb-1,:)];
...
%%
mpcpv = (cpv(:,2:ngpb,:) - cpv(:,1:ngpb-1,:))./ repmat(bdelta(1:ngpb-1)',ngpa,1,ngpy);
mpcpv = [mpcpv mpcpv(:,ngpb-1,:)];
Empcpv = sum(sum(sum(mpcpv.*gjoint.*repmat(abdelta,1,1,ngpy))));
Empcpv_by_y = squeeze(sum(sum(mpcpv.*gjoint.*repmat(abdelta,1,1,ngpy))))./ydist;
        

%%
mpcum1 = (ccum1(:,2:ngpb,:) - ccum1(:,1:ngpb-1,:))./ repmat(bdelta(1:ngpb-1)',ngpa,1,ngpy);
...
%%
mpcum4 = (ccum4(:,2:ngpb,:) - ccum4(:,1:ngpb-1,:))./ repmat(bdelta(1:ngpb-1)',ngpa,1,ngpy);
...
%%
mpd = (dep(:,2:ngpb,:) - dep(:,1:ngpb-1,:))./ repmat(bdelta(1:ngpb-1)',ngpa,1,ngpy);
mpd = [mpd mpd(:,ngpb-1,:)];
...
%%
rebamount = (500/50000).* (initss.Egrosslabinc*4);
...
%%
reblargeamount = (2500/50000).* (initss.Egrosslabinc*4);
...
if Save==1
    ...
    savefig([SaveDir '/mpc_quarterly_function']);
end
...
if Save==1
    ...
    savefig([SaveDir '/mpc_annual_function']);
end
